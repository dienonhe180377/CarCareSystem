        ...
        if ("list".equalsIgnoreCase(action)) {
            ...
            if (dateParam != null && !dateParam.trim().isEmpty()) {
                try {
                    ...
                    if ("marketing".equals(role) || "repairer".equals(role)) {
                        // Chỉ lấy điểm danh của chính user hiện tại
                        Attendance attendance = dao.getAttendanceByUserIdAndDate(currentUser.getId(), selectedDate);
                        attendanceList = new Vector<>();
                        if (attendance != null) {
                            attendanceList.add(attendance);
                        }
                        request.setAttribute("selectedDate", dateParam);
                        request.setAttribute("myAttendance", attendanceList);
                ...
                if ("marketing".equals(role) || "repairer".equals(role)) {
                    attendanceList = dao.getAttendanceByUserId(currentUser.getId());
                    request.setAttribute("myAttendance", attendanceList);
                    request.getRequestDispatcher("Attendance/MyAttendance.jsp").forward(request, response);
                    return;
                } else {
            ...
            // Nếu tới đây thì là Manager/Admin
...
//                        NOTIFICATION
                ...
                for (int i = 0; i < userList.size(); i++) {
                    if (userList.get(i).getUserRole().equals("manager")) {
                        ...
                        if (notiSetting.isEmail() && notiSetting.isAttendance()) {
