...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userA == null) {
            ...
            session.setAttribute("roleID", userA.getUserRole().toLowerCase());

            // Điều hướng theo role
            String role = userA.getUserRole().toLowerCase();
            switch (role) {
                case "admin":
                ...
                default:
                    session.invalidate();
                    request.setAttribute("error", "Bạn không có quyền truy cập hệ thống.");
                    request.getRequestDispatcher("/views/auth/login.jsp").forward(request, response);
                    break;
            }
        }
